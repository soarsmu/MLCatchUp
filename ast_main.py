...
# Simple file open test version

DEPRECATED_API_NAME = "sklearn.cluster.KMeans"
DEPRECATION_TYPE = "remove_keyword_param"
DEPRECATED_API_KEYWORD = []
default_test = "ast_test.py"
sklearn_dummyclassifier_file = "../ast_test.py"

with open("../working_testcase/add_parameter_test.py", "r", encoding="utf-8") as source:
...
list_api, import_dict, from_import_dict = process_api_format(tree, DEPRECATED_API_NAME)
...
# nameTransformer = ApiNameTransformer("sklearn.cluster.KMeans", "sklearn.cluster.SpectralCoclustering", list_line_number, list_completed_api)
nameTransformer = ApiNameTransformer("torch.tanh", "torch.nn.functional.tanh", list_line_number, list_completed_api)
...
# paramNameChange = KeywordParamChanger("torch.floor_divide", "input", "self", list_line_number)
# paramNameChange.transform(tree)

# positionalToKeyword = PositionalToKeyword("torch.add", 2, "alpha", list_line_number)
# positionalToKeyword.transform(tree)


addParameter = AddNewParameter("sklearn.cluster.KMeans", "new_parameter", "Call", "some_function_call()", list_line_number)
addParameter.transform(tree)