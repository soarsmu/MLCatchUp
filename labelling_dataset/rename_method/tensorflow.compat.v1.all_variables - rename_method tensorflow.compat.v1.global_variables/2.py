...
#
# In[2]:
# sess = tf.InteractiveSession()  # 历史版本
...
print(tf.argmax(a, 0).eval())  # 在每列中比较，找出每列中最大值所在的行号
print("argmax over axis 1")
print(tf.argmax(a, 1).eval())  # 在每行中比较，找出每行中最大值所在的列号
print("- " * 30)
...
# normal = tf.random_normal([3], stddev=0.1) # 历史版本
normal = tf.random.normal([3], stddev=0.1)
print(normal.eval())
print(normal.eval())
print("- " * 30)
...
print(var.name)
# tf.initialize_all_variables().run() # 历史版本
...
# for var in tf.all_variables(): # 历史版本
...
# v = tf.get_variable('v', shape=[2, 3], initializer=my_initializer) # 历史版本
v = tf.compat.v1.get_variable('v', shape=[2, 3], initializer=my_initializer)
# tf.initialize_all_variables().run() # 历史版本
tf.compat.v1.global_variables_initializer().run()
print(v.eval())
print("- " * 30)
...
with tf.compat.v1.variable_scope('layer1'):
    w = tf.compat.v1.get_variable('v', shape=[2, 3], initializer=my_initializer)
    print(w.name)
...
with tf.compat.v1.variable_scope('layer1', reuse=True):
    w = tf.compat.v1.get_variable('v')  # Unlike above, we don't need to specify shape and initializer
    print(w.name)
...
y = tf.compat.v1.placeholder(tf.int16)
