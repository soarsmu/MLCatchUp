...
class unet(object):
    def __init__(self):
        ...
        self.w_class = ast.literal_eval(params['class_weights'])  # class coef. for weighted loss
        ...
        self.progress = params['steps_to_display']  # averaged metrics to show after X steps
        self.loss_thr = params['loss_threshold']  # loss value below which early stop is considered
        self.valid_loss_thr = params['valid_threshold']  # same but for validation
        self.loss_diff = params['loss_difference']  # loss difference above which early stop is considered
        self.early_stop = params['steps_to_stop']  # epochs to trigger early stop if loss_diff does not decrease
        self.chkpt_path = params['checkpoint']  # checkpoint folder path
        self.res_name = params['model_to_restore']  # model name to be restored | to refine previous training
        self.model_path = params['model_path']  # path of model to restore
        self.restore = ast.literal_eval(params['restore_model'])  # restore model if True
        ...
        self.logits = layers.build_UNet(input=self.x,
                                        is_training=True,
    ...
    def loss(self, logits):
        ...
        assert (len(logits) == len(self.w_class)), \
            "Number of classes ({0}) iand weight coefficients ({1}) not matching.".format(len(logits),
                                                                                          len(self.w_class))

        epsilon = tf.constant(value=1e-10)      #to avoid division by 0
        with tf.name_scope('loss'):
            ...
            for i in range(self.n_class):
                ...
                lbls = tf.one_hot(tf.cast(labels[:, i], tf.int32), depth=2)     # convert to one-hot vector

                cost += tf.math.divide(tf.reduce_mean(
                    tf.multiply(tf.nn.softmax_cross_entropy_with_logits(logits=lgts, labels=lbls), self.w_class[i])),
                    float(self.n_class))
    ...
    def save_parameters(self, filename, sess, step):
    ...
    def test(self, test_data, restore=False):
        ...
        with tf.Session() as sess:
            ...
            for i in range(n):
                ...
                # get metrics and average metrics
                ...
                print("Input {0:.0f}, Loss: {1:.3f} --- f1: {2} --- "
                      "recall: {3:.3f} --- precision: {4:.3f} --- "
                      "specificity: {5:.3f}".format(int(idx), loss_, f1_txt, rec_, prec_, spec_))

        avg_f1_txt = self.array_to_text(avg_f1, 3)
        print("\nAvg. loss: %.3f --- Avg. f1: " % avg_loss + avg_f1_txt)
        print("Avg. recall: {0:.3f} --- Avg. precision: {1:.3f} --- "
              "Avg. specificity: {2:.3f}".format(avg_rec, avg_prec, avg_spec))
            ...
            for epoch in range(1, self.tot_epoch + 1):
                ...
                for i in range(iterations):
                    ...
                    feed_dict_tr = {self.x: x_batch,
                                    ...
                                    self.rate: self.dropout,
                                    self.learning_rate_: self.lr}
                    ...
                    if i % int(training_data._num_examples / self.batch_size) == 0 or i == (iterations - 1):
                        self.save_parameters(filename='segmentation.ckpt', sess=sess, step=i)


                # Epoch tasks
                # Calculate and display average metrics
                avg_f1_txt = self.array_to_text(avg_f1, 3)
                msg = "\nEpoch {0} / {1} (lr_{2}) --- Avg_loss: {3:.5f} --- " \
                      "Avg_f1: {4} --- Avg_rec: {5:.5f} --- Avg_prec: {6:.5f} --- Avg_acc: {7:.5f}"
                print(msg.format(epoch, self.tot_epoch, self.lr, avg_loss, avg_f1_txt, avg_rec, avg_prec, avg_acc))
                # Write metrics
                cont = 'Epoch ' + str(epoch) + ": " + str(avg_loss) + "; f1: " + \
                       avg_f1_txt + "; rec_mean: " + str(avg_rec) + "; prec_mean: " + \
                       str(avg_prec) + "; acc_mean: " + str(avg_acc) + "\n"
                ...
                msg = "Validation --- Avg. loss: {0:.5f} --- Avg_f1: {1} --- " \
                      "Avg_rec: {2:.5f} --- Avg_prec: {3:.5f} --- Avg_acc: {4:.5f}"
                print(msg.format(avg_loss_val, f1_val_txt, avg_rec_val, avg_prec_val, avg_acc_val))

                # Write metrics
                cont_val = 'Epoch ' + str(epoch) + ": " + str(avg_loss_val) + "; f1: " + \
                           f1_val_txt + "; rec_mean: " + str(avg_rec_val) + "; prec_mean: " + \
                           str(avg_prec_val) + "; acc_mean: " + str(avg_acc_val) + "\n"
                ...
                # Early stopping
                # Stop training if the loss difference between training and validation is
                # (1) beyond an arbitrary threshold, and (2) not decreasing over X epochs.

                # Loss difference
                loss_d = np.abs(avg_loss_val - avg_loss)

                # Conditions to stop
                if (stopping_step >= self.early_stop) and (avg_loss < self.loss_thr) and \
                        (avg_loss_val < self.valid_loss_thr) and (loss_d >= self.loss_diff):
                    print("\n------")
                    print("\nEarly stopping triggered at epoch: {0} --- loss: {1:.3f} --- validation: {2:.3f} --- "
                          "loss difference: {3:.3f}".format(epoch, avg_loss, avg_loss_val, loss_d))
                    self.save_parameters(filename='segmentation_earlystop.ckpt', sess=sess, step=epoch)
                    # return to break multiple loops
                    return

                # Keep track of loss difference
                if loss_d < self.loss_diff:
                    # if it decreases, reset epoch counter and continue
                    stopping_step = 0
                else:
                    # else, increase epoch counter until early_stop threshold
                    stopping_step += 1
        ...
        with tf.Session() as sess:
            ...
            for i in range(imgs.shape[0]):
                ...
                for clss in range(self.n_class):
                    ...
                    # save predictions
                    img_name = folder_name + '_input-' + str(i) + '_class-' + str(clss) + '_output-' + str(
                        output) + '.png'
